...
import com.chenlx.crouter_init.thread.thread.DefaultPoolExecutor;
...
public class ClassUtils {
    ...
    public static final String TAG = "CRouter_classutils";
     ...
     * @return 所有class的集合
     */
    public static Set[] getFileNameByPackageName(Context context,
                                                 @NonNull final String packageName,
                                                 @NonNull final String excludePackage,
                                                 @NonNull final String apiPath) throws PackageManager.NameNotFoundException, IOException, InterruptedException {

        Set[] sets = new Set[2];
        final Set<String> classNames = new HashSet<>();
        final Set<String> apiClassNames = new HashSet<>();
                ...
                public void run() {
                    ...
                    try {
                        if (path.endsWith(EXTRACTED_SUFFIX)) {
                            //NOT use new DexFile(path), because it will throw "permission error 
                        ...
                        while (dexEntries.hasMoreElements()) {
                             ...
                             * 过滤
                             */
                            if (excludePackage.equals(className)) {
                                continue;
                            }

                            if (className.startsWith(apiPath)) {
...
//        通过是否存在sp中的multidex.version是不准确的，因为从低版本升级上来的用户，是包含这个sp配置的
    ...
    private static List<String> tryLoadInstantRunDexFile(ApplicationInfo applicationInfo) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && null != applicationInfo.splitSourceDirs) {
            ...
            try {
                // This man is reflection from Google instant run sdk, he will tell me where the 
                // dex files go.
    ...
    private static boolean isVMMultidexCapable() {
        ...
        Log.i(TAG, "VM with name " + vmName + (isMultidexCapable ?
                " has multidex support" : " does not have multidex support"));
